...
export default function AddFoodScreen() {
    ...
    const { recentFoods, createMealSession, mealSessions } = useNutritionStore();

    // Route params: sessionId (add-more flow) + mealType override
    const params = useLocalSearchParams<{ sessionId?: string; mealType?: string }>();
    const targetSessionId = params.sessionId ? Number(params.sessionId) : undefined;

    // If adding to an existing session, find its name for the header
    const existingSession = targetSessionId
        ? mealSessions.find(s => s.id === targetSessionId)
        : undefined;

    const [mealType, setMealType] = useState<string>(
        params.mealType ?? getMealTypeByTime()
    );
    ...
    useEffect(() => {
        if (selectedFood) {
            Animated.spring(overlayAnim, { toValue: 1, useNativeDriver: true, tension: 50, friction: 8 }).start();
        } else {
    ...
    const loadData = async () => {
        if (!user?.id) return;
        const [cf, sm] = await Promise.all([getUserCustomFoods(user.id), getSavedMeals(user.id)]);
        setCustomFoods(cf);
        setSavedMeals(sm);
    };

    const combinedFoods = useMemo(() => [
        ...(Array.isArray(foodDatabase) ? foodDatabase : (foodDatabase as any).foods).map((f: any) => ({
            id: f.id,
            name: f.name,
            category: f.category,
            isCustom: false,
        ...
        ...customFoods.map(f => ({
            ...
            category: 'Custom',
            isCustom: true,
            per100g: { calories: f.calories, protein: f.protein, carbs: f.carbs, fats: f.fats },
        })),
    ], [customFoods]);
    ...
    const handleAddFoodToLog = (quantity: number, unit: 'g' | 'pc') => {
        ...
        if (unit === 'pc' && selectedFood.servingOptions?.length > 0) {
    ...
    const handleSaveCustomFood = async (food: any) => {
        ...
        setSelectedFood({
            ...
            category: 'Custom',
            isCustom: true,
            per100g: { calories: food.calories, protein: food.protein, carbs: food.carbs, fats: food.fats },
        });
        setSource('search');
    };
    ...
    // Create (or extend) a named meal session with all added items
    const handleFinish = async (sessionName: string) => {
        if (isSubmitting || addedItems.length === 0) return;
        ...
        try {
            await createMealSession(
                ...
                mealType,
                addedItems,
                targetSessionId   // undefined = new session, number = add to existing
            );
            router.back();
        } catch (e) {
    ...
    const handleSaveAsTemplate = () => {
        Alert.prompt('Save Template', 'Enter a name for this meal template', async (name) => {
            if (name && user?.id) {
    ...
    const headerTitle = existingSession
        ? `Add to "${existingSession.name}"`
        : 'Add Food';

    // Default session name: existing session name, or time-based meal type
    const defaultSessionName = existingSession?.name ?? capitalize(getMealTypeByTime());
        ...
        <View style={[styles.container, { backgroundColor: colors.background.primary }]}>
            ...
            <View style={[styles.content, { paddingTop: 8 }]}>
                ...
                <View style={[styles.tabBar, { backgroundColor: colors.background.elevated, borderColor: colors.border.primary }]}>
                    {([
                        ...
                        <TouchableOpacity
                            key={tab.key}
                            style={[styles.tabPill, source === tab.key && { backgroundColor: colors.accent.primary + '18' }]}
                            onPress={() => { Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); setSource(tab.key); }}
                        >
                            <Ionicons
                                name={tab.icon}
                                size={16}
                ...
                {source === 'search' && (
                    <FoodSearch
                        ...
                        recentFoods={recentFoods.slice(0, 8).map(f => ({
                            id: `recent-${f.id}`,
                            name: f.name,
                            category: 'Recent',
                            isCustom: false,
                ...
                {source === 'custom' && (
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                        <CustomFoodForm onSave={handleSaveCustomFood} onCancel={() => setSource('search')} />
                    </ScrollView>
                )}

                {source === 'saved' && (
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
                        <SavedMealsGrid meals={savedMeals} onSelect={handleSelectSavedMeal} onLongPress={() => {}} />
                    </ScrollView>
                )}
            </View>
                ...
                <View style={styles.overlay}>
                    <Animated.View style={[StyleSheet.absoluteFill, { opacity: overlayAnim }]}>
                        <BlurView intensity={20} tint="dark" style={StyleSheet.absoluteFill}>
                            <TouchableOpacity style={StyleSheet.absoluteFill} activeOpacity={1} onPress={() => setSelectedFood(null)} />
                        </BlurView>
                    </Animated.View>
                    <Animated.View style={[styles.overlayContent, {
                        opacity: overlayAnim,
                        transform: [{ scale: overlayAnim.interpolate({ inputRange: [0, 1], outputRange: [0.9, 1] }) }],
                    }]}>
                        <QuantityCard food={selectedFood} onAdd={handleAddFoodToLog} onCancel={() => setSelectedFood(null)} />
                    </Animated.View>
                </View>
            )}
            ...
            <Modal visible={showSourceSelector} transparent animationType="slide" onRequestClose={() => setShowSourceSelector(false)}>
                <View style={styles.modalOverlay}>
                    <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={() => setShowSourceSelector(false)} />
                    <SourceSelector
                        selectedMealType={mealType}
                        onSelectMealType={setMealType}
                        onSelectSource={(s) => { setSource(s); setShowSourceSelector(false); }}
                        onClose={() => setShowSourceSelector(false)}
                    />
                </View>
            </Modal>

            {/* Review card with session name */}
            <FloatingReviewCard
                items={addedItems}
...
const styles = StyleSheet.create({
    ...
    headerBtn: { padding: 8 },
    tabBar: { flexDirection: 'row', borderRadius: 14, padding: 4, marginBottom: 16, borderWidth: 1 },
    tabPill: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 10, borderRadius: 11 },
    tabLabel: { fontSize: 12, fontWeight: '600' },
    overlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', padding: 20, zIndex: 1000 },
    overlayContent: {},
